...
class Solution:
    def shortestPathLength(self, graph: List[List[int]]) -> int:
        ...
        curr_level = deque([(u, 1 << u) for u in range(graphLength)])  # BFS Queue

        # Calculate a bitmask where all vertices are visited.
        all_visited = (1 << graphLength) - 1  # Bitmask for when all vertices are visited

        # Initialize a 2D array to track visited nodes.
        visited = [[False for bit_mask in range(0, all_visited + 1)] for _ in range(graphLength)]

        # Mark the initial states of the BFS by setting corresponding cells in the visited array to True.
        for u in range(graphLength):
            visited[u][1 << u] = True
